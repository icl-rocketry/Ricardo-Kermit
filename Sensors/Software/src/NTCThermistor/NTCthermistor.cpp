...
NTCThermistor::NTCThermistor(ADS1219 &ADS, const uint32_t Rfixed, const uint32_t Rseriesextra, const float T1, const float R1, const float Beta, const float calfactor, uint8_t ADCchannel,RnpNetworkManager& netman) : 
NRCRemoteSensorBase(netman),
_ADS(ADS),
RFixedPD(Rfixed),
Rseries(Rseriesextra),
...
float NTCThermistor::calculateLinear()
{
    Rtherm = RFixedPD * ((((float)ADCMax) * (float)calfactor / ((float)_ADS.readAdjusted(channel)) - 1.0)) - Rseries;
...
{
    updateSensorValue(calculateLinear());
}