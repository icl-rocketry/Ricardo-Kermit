...
void ADS1219::begin(adsGain_t gain, uint rate, adsMode_t mode, adsRef_t vref, int Offset)
{
  setGain(gain);
  setDataRate(rate);
...
long ADS1219::getOffset(const uint readingNumber, const uint timeincrement)
{
  iter = 0;
  doffset = 0;
  readShorted();
